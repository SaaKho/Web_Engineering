...
app.post('/CommentonBlog/:id', async (req, res) => {
    ...
    try {
        // Find the blog post by ID and update the comments array
        const blogPost = await Blog.findById(postId);
        if (!blogPost) {
            return res.status(404).json({ error: 'Blog post not found' });
        }

        blogPost.comments.push(comment); // Add the comment to the comments array
